    ...
    class booking_package_schedule {
			...
			foreach ((array) $rows as $row) {
				...
				if ($calendarAccount['type'] == 'day') {
					...
					foreach ((array) $services as $service) {
						...
						/**
						array_push($visitor['services'], $service['name']);
						$visitor['amount'] += intval($service['cost_1']);
						foreach ((array) $service['options'] as $option) {
							
							if (intval($option['selected']) == 1) {
								
								array_push($visitor['services'], $option['name']);
								$visitor['amount'] += intval($option['cost_1']);
    	...
    	public function createAccommodationDetails($originCalendarKey, $calendarAccountKey, $json, $sql_start_unixTime, $applicantCount, $type, $accommodationDetails = null){
    		...
    		if (count($scheduleList) != 0) {
    			...
    			$table_name = $wpdb->prefix."booking_package_schedule";
    			$sql = $wpdb->prepare(
    				"SELECT `key`,`month`,`day`,`year`,`weekKey`,`unixTime`,`cost`,`remainder` FROM `".$table_name."` WHERE `accountKey` = %d AND (`unixTime` >= %d AND `unixTime` <= %d) AND `status` = 'open' ORDER BY `unixTime` ASC;", 
			...
			if (is_array($emails)) {
				...
				for ($i = 0; $i < count($emails); $i++) {
					...
					if (!is_null($endUnix)) {
						...
						$response['message'] = sprintf(__('You have already booked between %s and %s by %s.'), $this->dateFormat($dateFormat, $positionOfWeek, $startUnix, '', true, true, 'text'), $this->dateFormat($dateFormat, $positionOfWeek, $endUnix, '', true, true, 'text'), $email);
						...
						$response['message'] = sprintf(__('You have already booked at %s by %s.'), $this->dateFormat($dateFormat, $positionOfWeek, $startUnix, '', true, true, 'text'), $email);
		...
		public function sendVerificationCode($administrator = false) {
			...
			$calendarAccount = $this->getCalendarAccount($accountKey);
			if ($calendarAccount['bookingVerificationCode'] != 'false') {
				
				$response_user = $this->get_user_id($administrator, $_POST['userId']);
				$userValues = $this->getUserValues($accountKey, $administrator, 'add', null, $response_user['user_id']);
				...
				$sms = $userValues['sms'];
				if ($calendarAccount['bookingVerificationCode'] == 'emailAndSms' || $calendarAccount['bookingVerificationCode'] == 'email') {
				...
				if ($calendarAccount['bookingVerificationCode'] == 'emailAndSms' || $calendarAccount['bookingVerificationCode'] == 'sms') {
		...
		public function sendBooking($administrator = false) {
			...
			if (is_null($row)) {
				...
				if ($row['unixTime'] == $scheduleUnixTime) {
					...
					$privateResponse = $this->insertPrivateData($sendDate, $_POST['permission'], $status, $_POST['timeKey'], $scheduleUnixTime, $scheduleTitle, $scheduleCost, $services, $form, $emails, $currency, $_POST['payType'], $cardToken, $accountKey, $permalink, $preparation, $taxes, $responseGuests, $coupon, $administrator, $applicantCount);
					...
					if (isset($_POST['payToken'])) {
						...
						if (intval($payment_active) == 1 && !empty($secret_key)) {
							...
							$payResponse = $creditCard->pay($_POST['payType'], $public_key, $secret_key, $_POST['payToken'], $payment_live, $totalCost, $currency, $lastID, $visitorName, $visitorEmail, $visitorBookingDate);
		...
		public function getSelectedGuests($calendarAccount, $selectedGuestsString, $mode) {
			...
			for ($guestKey = 0; $guestKey < count($selectedGuests); $guestKey++) {
				...
				if (is_null($row)) {
					...
					if (isset($list[$key]) && $list[$key]['name'] == $selectedGuest['selectedName']) {
		...
		public function getUserValues($accountKey, $type, $administrator, $personalInformation = null, $user_id = null) {
			...
			if ($type == 'update' && empty($personalInformation) === false) {
				...
				$data = json_decode($personalInformation);
			...
			for ($i = 0; $i < count($form); $i++) {
				...
				if (isset($_POST['form' . $i])) {
					...
					if (($form[$i]->required == 'true' || $form[$i]->required == 'true_frontEnd') && strlen(preg_replace("/( |　)/", "", $_POST['form' . $i])) == 0) {
    	...
    	public function createEmailMessage($accountKey, $email_id, $form, $accommodationDetails, $options, $bookingID, $unixTime, $timestampForUnixTime, $cancellationUri, $currency = 'usd', $services = null, $payName = null, $payId = null, $scheduleTitle = null, $guests = null, $coupon = null){
			...
			foreach ((array) $sendEamilList as $emailKey => $target) {
				...
				foreach ((array) $emailData as $emailDataKey => $emailDataValue) {
					...
					$timestamp = $this->dateFormat($dateFormat, $positionOfWeek, $timestampForUnixTime, $scheduleTitle, true, false, 'text');
					...
					if (preg_match('/(\[checkIn\])/', $emailDataValue, $matches)) {
						...
						$date = $this->dateFormat($dateFormat, $positionOfWeek, $accommodationDetails['checkIn'], $scheduleTitle, false, false, 'text');
					...
					if (preg_match('/(\[checkOut\])/', $emailDataValue, $matches)) {
						...
						$date = $this->dateFormat($dateFormat, $positionOfWeek, $accommodationDetails['checkOut'], $scheduleTitle, false, false, 'text');
						...
						if (preg_match('/(\[service\])/', $emailDataValue, $matches)) {
							...
							if (is_array($services)) {
								...
								foreach ((array) $services as $key => $service) {
									...
									$responseCostInService = $this->getCostsInService($calendarAccount, $service, $guestsList);
									#var_dump($responseCostInService);
									$costs = $responseCostInService['costs'];
									$details = $service['name'];
									if (is_int(intval($costs[0])) === true && intval($responseCostInService['max']) != 0) {
										
										#$details .= ' ' . $this->formatCost($service['cost'], $currency);
					...
					for ($i = 0; $i < count($form); $i++) {
						...
						if ($form[$i]->isSMS == 'true' && !empty($form[$i]->value)) {
				...
				$responseList = array('body' => $emailBody, 'to' => $to, 'from' => $from, 'sendVisitor' => $sendVisitor, 'headers' => $headers, 'visitorEmail' => $visitorEmail, 'response' => array(), 'params' => array(), 'enabledSMS' => $enableSMS);
		...
		public function twilioSMS($visitorSMS, $body) {
