...
*/
	...
	class BOOKING_PACKAGE {
		...
		public function __construct($shortcodes = 0) {
			...
			**/
			...
			if (function_exists('register_activation_hook')) {
				...
				register_activation_hook(__FILE__, array($this,'booking_notification'));
				
			}
			
			if (!wp_get_schedule('booking_notification')) {
				
				wp_schedule_event(time(), 'hourly', 'booking_notification');
			...
			add_action('booking_notification', array($this, 'do_booking_notification'));
		...
		public function dashboard_widget_function(){
			...
			$unixTime = date('U');
			$today = $schedule->dateFormat($dateFormat, date('w'), $unixTime, null, null, false, 'text');
			...
			$unixTime += 1440 * 60;
			$nextDay = $schedule->dateFormat($dateFormat, date('w', $unixTime), $unixTime, null, null, false, 'text');
			$lists = $schedule->getReservationUsersData(date('n', $unixTime), date('j', $unixTime), date('Y', $unixTime));
		...
		public function bookingPageForVisitors($atts){
			...
			if (isset($_SESSION['verificationCode'])) {
				...
				var_dump($_SESSION['verificationCode']);
			...
			**/
			...
			$_REQUEST['shortcodes_for_booking_package'] = $this->shortcodes;
			if ($this->shortcodes > 1) {
				
				print '<div class="shortcode_error"><p>';
				print __('Shortcode of the Booking Package cannot insert multiple shortcodes on one page.', $pluginName) . '<br>';
				print sprintf(__('ID: %s has canceled the display.', $pluginName), $id);
				print '</p></div>';
				return null;
		...
		public function adomin() {
			...
			if (isset($_GET['mode']) && $_GET['mode'] == 'upgradePlan') {
				...
				$response = $setting->getSiteStatus(false);
		...
		public function schedule_page(){
			...
			$isExtensionsValid = $setting->getSiteStatus(true);
			if ($isExtensionsValid == true) {
			...
			<div id="select_package" style='display: none;'>
				...
				<div class="wrap">
					...
					<div id="guestsDisabled" class="hidden_panel"><?php _e("Guests function is disabled. ", $pluginName); echo " "; _e("Please change the setting on the \"Setting\" tab.", $pluginName); ?></div>
					-->
					...
					<div class="edit_modal_content">
						...
						<div id="email_edit_panel" class="media_left_zero hidden_panel">
							...
							<div class="mail_message_area_left">
								<div class="enablePanel">
									...
									<div class="enableValuePanel">
										...
										if ($this->twilioSMS == 1) {
											...
											?>
											<label class="hidden_panel"><input type="checkbox" id="smsEnable"/><?php _e("SMS", $pluginName); ?></label>
											<?php
								...
								<div class="emailFormatLabel" style="padding-bottom: 10px;"><?php _e("Message for visitor", $pluginName); ?></div>
								<div class="emailFormatPanel">
								...
								<div class="emailFormatLabel" style="padding: 10px 0; width: inherit;"><?php _e("Message for administrators", $pluginName); ?></div>
								<div class="emailFormatPanel">
				...
				<div id="copyAndPasteOnCalendarSetting" class="hidden_panel"><?php _e('Copy a shortcode and paste it in your page on the Dashboard > Pages.', $pluginName); ?></div>
		...
		public function localizeScript($mode){
			...
			if ($mode == 'adomin') {
				...
				if ($this->bookingVerificationCode == 0) {
					...
					unset($elementForCalendarAccount['bookingVerificationCode']);
				...
				$localize_script = array(
					...
					'today' => date('Ymd'),
					'bookingVerificationCode' => $this->bookingVerificationCode,
				);
		...
		**/
		...
		public function deactivation_event(){
			...
			wp_clear_scheduled_hook('booking_notification');
		...
		public function getDictionary($mode, $pluginName){
			...
			$dictionary = array(
				...
				**/
				...
				'Remainders' => __('Remainders', $pluginName),
			...
			if ($mode == "adomin") {
				...
				$dictionary['Schedule every %s'] = __('Schedule every %s', $pluginName);
